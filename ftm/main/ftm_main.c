                               ...
                               int32_t event_id, void *event_data)
{
    ...
    if (event->status == FTM_STATUS_SUCCESS) {
        ESP_LOGI(TAG_STA, "RTT=%d nSec, raw=%d nsec Distance -> %d.%02d m", event->rtt_est, event->rtt_raw,
...
static int wifi_cmd_loop(int argc, char **argv)
{
    int nerrors = arg_parse(argc, argv, (void **) &loop_args);
    ...
    if (nerrors != 0) {
        arg_print_errors(stderr, loop_args.end, argv[0]);
        return 0;
    }

    ap_record = find_ftm_responder_ap(loop_args.ssid->sval[0]);
    if (!ap_record)
        {
        printf("Could not find AP\n");
        return 0;
        }

    printf("FTM to " MACSTR " on ch %d\n", MAC2STR(ap_record->bssid),
                                    ap_record->primary);
    ...
    for(int aa = 0; aa < 1000; aa++)
        ...
        EventBits_t bits = xEventGroupWaitBits(ftm_event_group, FTM_REPORT_BIT | FTM_FAILURE_BIT,
                                               pdFALSE, pdFALSE, 3000);  // portMAX_DELAY
        /* Processing data from FTM session */

        // Light travels 0.299792458 meters per nanosecond
        ...
        if (bits & FTM_REPORT_BIT) {
            for (int i = 0; i < g_ftm_report_num_entries; i++) {
                ...
                //                g_ftm_report[i].t1 / 1000LL,
                //                    g_ftm_report[i].t2 / 1000LL,
                //                        g_ftm_report[i].t3 / 1000LL,
                //                            g_ftm_report[i].t4 / 1000LL);

                int64_t d1 = (g_ftm_report[i].t3) - (g_ftm_report[i].t2);
                int64_t d2 = (g_ftm_report[i].t4) - (g_ftm_report[i].t1);

                int ddiff = d2-d1 - 7000; // This is the reding for the zero distance
            ...
            for (int i = 0; i < g_ftm_report_num_entries; i++) {
                ...
                if(ABS(dev) < dsum / 20)
                    {
                    //printf("[ %d roll %d]", darr[i], ABS(dev));
                    dsum2 += darr[i];  cnt++;
                    }
                else
                    {
            ...
            float meters = (float)dsum2 * (0.299792458 / 2000); // We have picoseconds * round trip
            printf("\navg dsum: %d dsum2: %d   %.3f meters\n", dsum, dsum2, meters);
...
ftm_start:
    ...
    /* Processing data from FTM session */
    if (bits & FTM_REPORT_BIT) {
        int i, sum2 = 0, cnt = 0, sum = 0, dev = 0;
        for (i = 0; i < g_ftm_report_num_entries; i++) {
            /* NOTE: Process FTM report elements here, e.g. g_ftm_report[i]->rtt etc */
            //printf("rt:%d rs:%d  ", g_ftm_report[i].rtt, g_ftm_report[i].rssi);
            //printf("t1:%lld t2:%lld t3:%lld t4:%lld\n",
            //                g_ftm_report[i].t1,
            //                    g_ftm_report[i].t2,
            //                        g_ftm_report[i].t3,
            ...
            //printf("t2-t1=%lld t4-t3=%lld     ",
            //                    g_ftm_report[i].t2 -
            //                        g_ftm_report[i].t1,
            //                            g_ftm_report[i].t4 -
            //                                g_ftm_report[i].t3);

            sum +=  g_ftm_report[i].rtt;
        }
    sum /= g_ftm_report_num_entries;
    for (i = 0; i < g_ftm_report_num_entries; i++)
        {
        dev =  g_ftm_report[i].rtt - sum;
        //printf("dev=%d abs=%d ", dev, ABS(dev));
        if(ABS(dev) < 500)
            {
            //printf("[roll %d]", dev);
            sum2 += g_ftm_report[i].rtt; cnt++;
            }
        }
    if(cnt)
        sum2 /= cnt;

    printf("my avg: %d rolling: %d\n", sum, sum2);

    printf("rtt:%d rsi:%d ", g_ftm_report[i].rtt, g_ftm_report[i].rssi);
...
void register_wifi(void)
{
    ...
    loop_args.ssid = arg_str0("s", "ssid", "SSID", "SSID of AP");
    loop_args.end = arg_end(1);

    const esp_console_cmd_t loop_cmd = {
        .command = "loop",
        .help = "Exec FTM in a loop",
        .hint = NULL,
        .func = &wifi_cmd_loop,
        .argtable = &loop_args
    };

    ESP_ERROR_CHECK( esp_console_cmd_register(&loop_cmd) );
}
