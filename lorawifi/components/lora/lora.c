 ...
 * @param level 2-15, from least to most power
 */
...
lora_set_bandwidth(long sbw)
{
   ...
   if (sbw <= 7.8E3) gl_bw = 0;
   else if (sbw <= 10.4E3) gl_bw = 1;
   else if (sbw <= 15.6E3) gl_bw = 2;
   else if (sbw <= 20.8E3) gl_bw = 3;
   else if (sbw <= 31.25E3) gl_bw = 4;
   else if (sbw <= 41.7E3) gl_bw = 5;
   else if (sbw <= 62.5E3) gl_bw = 6;
   else if (sbw <= 125E3) gl_bw = 7;
   else if (sbw <= 250E3) gl_bw = 8;
   else gl_bw = 9;

   //printf("Set swb %ld gl_bw %d\n", sbw, gl_bw);

   lora_write_reg(REG_MODEM_CONFIG_1,
            (lora_read_reg(REG_MODEM_CONFIG_1) & 0x0f) | (gl_bw << 4));
}
...
lora_read_freq_err()
{
    ...
    uint8_t r3 = lora_read_reg(0x2a);

    //printf("Read freq err %d %d %d\n", r1 & 0xff, r2 & 0xff, r3 & 0xff);
    int ret = (r1 & 0xf) << 16 | r2 << 8 | r3;
    if(ret & (1 << 19))
        {
        //printf("neg: %d", ret);
        ret = -(ret & ((1 << 19) -1));
